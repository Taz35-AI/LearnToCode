...
with checks(item, ok, detail) as (
  ...
  select 'Course content loaded',
         (select count(*) from lessons) = 92
         and (select count(*) from quiz_questions) = 416
         and (select count(*) from lesson_blocks) = 1006,
  ...
  select 'Reviewable items loaded',
         (select count(*) from review_items) = 376
         and (select count(*) from review_items where kind = 'exercise') = 142
         and (select count(*) from review_items where kind = 'question') = 234,
  ...
  select 'Retrieval-practice blocks authored',
         ...
         || ' of 6 types'
  union all
  select 'All 24 levels and 33 modules',
         (select count(*) from levels) = 24 and (select count(*) from modules) = 33,
